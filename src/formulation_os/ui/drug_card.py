...
def render_drug_card(st, profile: dict[str, Any], *, live: bool = False) -> None:
    ...
    with st.expander(f"⑤ Patents & Exclusivity — patents {len(patents)} · exclusivity {len(excl)}  (Orange Book)"):
        for r in patents[:6]:
            st.markdown(f"- 📜 patent US {r.get('patent_number',{}).get('value')} "
                        f"exp {r.get('patent_expire_date',{}).get('value')}")
        for r in excl[:6]:
